...
export function playbackPage(
  ...
  const data = scriptJson({
    ...
    card,
    // Every part a story could be about, so the stage can offer the choice
    // without shipping the whole sheet twice.
    parts: storyParts(sheet.sections),
  });
...
<div class="story-stage" id="story-stage" hidden><canvas class="story-canvas" id="story-canvas" width="1080" height="1920"></canvas><div class="story-foot"><div class="story-track" id="story-track" role="slider" tabindex="0" aria-label="Where the story starts" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><div class="story-bands" id="story-bands"></div><div class="story-window" id="story-window"></div></div><span class="story-where" id="story-where"></span><div class="story-progress" id="story-progress" hidden><i id="story-bar"></i></div><span class="story-note" id="story-note">Keep this open while it records</span><button class="story-send" id="story-go" type="button">Record</button><button class="story-close" id="story-close" type="button">Close</button></div></div>
...
${STORY_CARD_SCRIPT}
let card=song.card;
const story=document.querySelector('#story'),storyLabel=story.querySelector('span'),parts=Array.isArray(song.parts)?song.parts:[],partSections=parts.map(part=>part.sectionIndex);
const stage=document.querySelector('#story-stage'),stageCanvas=document.querySelector('#story-canvas'),stageTrack=document.querySelector('#story-track'),stageBands=document.querySelector('#story-bands'),stageWindow=document.querySelector('#story-window'),stageWhere=document.querySelector('#story-where'),stageProgress=document.querySelector('#story-progress'),stageBar=document.querySelector('#story-bar'),stageNote=document.querySelector('#story-note'),stageGo=document.querySelector('#story-go'),stageClose=document.querySelector('#story-close');
const storyShareable=storyCard.canShareStoryCard(),storyResting=storyShareable?'Story':'Card';
const storyRecordable=storyShareable&&parts.length>0&&Boolean(storyCard.storyVideoType());
let storyDrawing=null,storyBusy=false,storySettle=0,storyVideo=null,storyPart=-1,storyClip={start:0,seconds:0},storyScrubbing=false,storyFrame=0;
...
function drawPreview(){if(storyFrame)return;storyFrame=requestAnimationFrame(()=>{storyFrame=0;const image=storyCard.readyStoryBackground(card.backgroundUrl);if(image!==null||!card.backgroundUrl)storyCard.drawStoryCard(stageCanvas,card,image);else storyCard.loadStoryBackground(card.backgroundUrl).then(loaded=>storyCard.drawStoryCard(stageCanvas,card,loaded)).catch(()=>{})})}
// The moment is what a person picks; the words follow it. The card can never
// quote one part of the song while the clip plays another.
function moveTo(seconds){const duration=songDuration();storyClip=storyCard.storyClipAt(seconds,duration);storyVideo=null;
const at=storyCard.storyPartAt(timeline,partSections,storyClip.start+storyClip.seconds/2,duration);
if(at>=0&&at!==storyPart){storyPart=at;card={...card,sectionIndex:parts[at].sectionIndex,stanza:parts[at].stanza};storyDrawing=null;drawPreview()}
const width=duration>0?(storyClip.seconds/duration)*100:100;
stageWindow.style.left=(duration>0?(storyClip.start/duration)*100:0)+'%';stageWindow.style.width=width+'%';
stageTrack.setAttribute('aria-valuenow',String(Math.round(duration>0?(storyClip.start/duration)*100:0)));
stageTrack.setAttribute('aria-valuetext',(parts[storyPart]?parts[storyPart].label+', ':'')+formatTime(storyClip.start));
stageWhere.textContent=(parts[storyPart]?parts[storyPart].label+' · ':'')+formatTime(storyClip.start);
...
function buildBands(){const duration=songDuration();stageBands.replaceChildren();if(!timeline||!(duration>0))return;
timeline.forEach(entry=>{const band=document.createElement('div');band.className='story-band';band.style.width=(((entry.end-entry.start)/duration)*100)+'%';const at=partSections.indexOf(entry.sectionIndex);if(at>=0){const name=document.createElement('span');name.textContent=parts[at].label;band.append(name)}stageBands.append(band)})}
function scrubTo(event){const box=stageTrack.getBoundingClientRect();if(!box.width)return;const ratio=Math.min(1,Math.max(0,(event.clientX-box.left)/box.width));moveTo(ratio*songDuration()-storyClip.seconds/2);
// Hearing where you are is the only way to aim without a waveform.
if(audio.readyState>0){audio.currentTime=storyClip.start;if(audio.paused)audio.play().catch(()=>{})}}
stageTrack.addEventListener('pointerdown',event=>{if(storyBusy)return;storyScrubbing=true;stageTrack.setPointerCapture(event.pointerId);scrubTo(event)});
stageTrack.addEventListener('pointermove',event=>{if(storyScrubbing)scrubTo(event)});
stageTrack.addEventListener('pointerup',()=>{storyScrubbing=false;audio.pause()});
stageTrack.addEventListener('pointercancel',()=>{storyScrubbing=false;audio.pause()});
stageTrack.addEventListener('keydown',event=>{const step=event.key==='ArrowLeft'?-5:event.key==='ArrowRight'?5:0;if(!step||storyBusy)return;event.preventDefault();moveTo(storyClip.start+step)});
...
stageGo.hidden=true;stageTrack.hidden=true;stageWhere.hidden=true;stageProgress.hidden=false;stageNote.hidden=false;stageNote.textContent='Keep this open · '+Math.round(clip.seconds)+'s';
try{const blob=await storyCard.recordStoryVideo({canvas:stageCanvas,card,audio,clipStart:clip.start,seconds:clip.seconds,onProgress:value=>{stageBar.style.width=(value*100).toFixed(1)+'%';stageNote.textContent='Keep this open · '+Math.ceil(clip.seconds*(1-value))+'s'}});
...
finally{stageTrack.hidden=false;stageWhere.hidden=false}}
...
function openStage(){audio.pause();buildBands();
...
moveTo(entry?entry.start:songDuration()*0.3);
storyCard.loadStoryBackground(card.backgroundUrl).then(()=>drawPreview()).catch(()=>{});
stage.hidden=false}
...
async function stillCard(){storyLabel.textContent='…';try{storySay(await storyCard.shareOrSaveStoryCard(await storyBlob(),card)==='saved'?'Saved':storyResting)}catch{storySay('Retry')}}
story.addEventListener('click',async()=>{if(storyBusy)return;storyBusy=true;clearTimeout(storySettle);
try{if(storyRecordable){audio.pause();buildChips();choosePart(storyPart);stage.hidden=false}else await stillCard()}
catch{closeStage();storySay('Retry')}
finally{storyBusy=false}});
</script></body></html>`;
}
