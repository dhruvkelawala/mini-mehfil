...
.lyric-line{opacity:.72;transition:color .35s,opacity .35s,text-shadow .35s,transform .35s}.lyric-line.lyric-line-upcoming{opacity:.42}.lyric-line.lyric-line-current{opacity:1;text-shadow:0 0 18px rgba(249,237,218,.3);transform:scale(1.015)}.lyric-line.lyric-cue{opacity:1}
...
<main class="performance"><div class="performance-content"><section class="lyric-reveal" aria-label="Lyrics"><strong class="reveal-language">${escapeHtml(label)}</strong><small class="performance-timing">Atmospheric reveal · timing is approximate</small><div class="reveal-lines" id="reveal-lines" aria-live="polite"></div></section><button class="performance-replay" id="replay" type="button" hidden>${REPLAY_ICON}<span>Replay the mehfil</span></button></div></main>
...
const audio=document.querySelector('#audio'),play=document.querySelector('#play'),seek=document.querySelector('#seek'),timecode=document.querySelector('#timecode'),linesRoot=document.querySelector('#reveal-lines'),timingNote=document.querySelector('.performance-timing'),replay=document.querySelector('#replay'),player=document.querySelector('#player-shell'),scene=document.querySelector('.scene'),share=document.querySelector('#share'),song=JSON.parse(document.querySelector('#song-data').textContent);
...
function makeSection(section){const element=document.createElement('span');element.className='lyric-section lyric-section-current';section.lines.forEach(value=>element.append(makeLine(value)));return element}
function activeEntry(currentTime){return Number.isFinite(currentTime)&&currentTime>=0&&timeline&&timeline.find(entry=>entry.start<=currentTime&&currentTime<entry.end)||null}
function activeLine(currentTime){return Number.isFinite(currentTime)&&currentTime>=0&&pacing.find(entry=>entry.start<=currentTime&&currentTime<entry.end)||null}
function emphasizeLine(section){const current=activeLine(audio.currentTime),lineElements=linesRoot.children[0]&&linesRoot.children[0].children||[];[...lineElements].forEach((element,index)=>{const value=section.lines[index],isCurrent=!value.cue&&current&&current.sectionIndex===section.index&&current.lineIndexInSection===index,isUpcoming=!value.cue&&current&&current.sectionIndex===section.index&&index>current.lineIndexInSection;element.className=value.cue?'lyric-line lyric-cue':'lyric-line'+(isCurrent?' lyric-line-current':isUpcoming?' lyric-line-upcoming':'');if(isCurrent)element.setAttribute('aria-current','true');else element.removeAttribute('aria-current')})}
function syncApproximate(){if(renderKey!=='approximate'){linesRoot.replaceChildren(...lines.map(makeLine));renderKey='approximate'}const duration=audio.duration,pacedDuration=duration*.9,progress=pacedDuration>0?Math.min(audio.currentTime/pacedDuration,1):0,spokenCount=lines.filter(line=>!line.cue).length,shown=Math.min(spokenCount,Math.ceil(progress*spokenCount));let seen=0;[...linesRoot.children].forEach((element,index)=>{const value=lines[index];if(!value.cue){element.hidden=++seen>shown;return}const trailing=seen===spokenCount;element.hidden=trailing?shown<spokenCount:shown<=seen})}
function syncSections(){const entry=activeEntry(audio.currentTime),nextKey=entry?entry.start+':'+entry.end+':'+entry.label+':'+entry.sectionIndex:'timed-rest',section=entry&&entry.sectionIndex!==null?sections.find(value=>value.index===entry.sectionIndex):null;if(renderKey!==nextKey){renderKey=nextKey;if(section)linesRoot.replaceChildren(makeSection(section));else if(entry&&(entry.label==='inst'||entry.label==='silence'))linesRoot.replaceChildren(makeLine({cue:true,primary:entry.label==='inst'?'Instrumental':'Pause',secondary:''}));else linesRoot.replaceChildren()}if(section)emphasizeLine(section)}
function validateTimelineDuration(){const duration=audio.duration,mappedTimeline=Array.isArray(song.timeline)?song.timeline:null,tolerance=Math.max(1,duration*.02),hasArtifact=Number.isFinite(song.expectedDurationSeconds)&&song.expectedDurationSeconds>0;timingValidated=Boolean(mappedTimeline&&Number.isFinite(duration)&&duration>0&&Math.abs(song.expectedDurationSeconds-duration)<=tolerance);timeline=timingValidated?mappedTimeline:null;timingNote.textContent=timingValidated?'Lines follow MiniMax sections · timing is approximate':'Atmospheric reveal · timing is approximate';renderKey='';const diagnostic={event:'shared-page-validation',surface:'shared-page',reason:!hasArtifact?'untimed':!mappedTimeline?'weak-map':timingValidated?'duration-match':'duration-mismatch',segmentCount:mappedTimeline?mappedTimeline.length:0,sectionCount:sections.length};if(Number.isFinite(song.expectedDurationSeconds))diagnostic.analyzedDurationSeconds=song.expectedDurationSeconds;if(Number.isFinite(duration))diagnostic.mediaDurationSeconds=duration;console.info('[TIMING-DIAGNOSTIC]',diagnostic)}
function sync(){if(timingValidated)syncSections();else syncApproximate();const duration=audio.duration;seek.value=duration?(audio.currentTime/duration)*100:0;timecode.textContent=formatTime(audio.currentTime)+' / '+formatTime(duration);linesRoot.scrollTop=linesRoot.scrollHeight}
let visualFrame=0;
