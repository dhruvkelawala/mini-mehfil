...
<div class="story-stage" id="story-stage" hidden><canvas class="story-canvas" id="story-canvas" width="1080" height="1920"></canvas><div class="story-foot"><div class="story-progress"><i id="story-bar"></i></div><span class="story-note" id="story-note">Recording your story…</span><button class="story-send" id="story-send" type="button" hidden>Share to your story</button><button class="story-close" id="story-close" type="button">Close</button></div></div>
...
const stage=document.querySelector('#story-stage'),stageCanvas=document.querySelector('#story-canvas'),stageBar=document.querySelector('#story-bar'),stageNote=document.querySelector('#story-note'),stageSend=document.querySelector('#story-send'),stageClose=document.querySelector('#story-close');
const storyShareable=storyCard.canShareStoryCard(),storyResting=storyShareable?'Story':'Card';
// A clip long enough to be worth recording, in a browser that records MP4.
const storyRecordable=storyShareable&&Boolean(storyCard.storyVideoType());
// The clip plays the section the card quotes, measured against the real audio
// rather than the duration the provider claimed.
function storyClipNow(){return storyCard.storyClip(timeline,card.sectionIndex,audio.duration)}
let storyDrawing=null,storyBusy=false,storySettle=0,storyVideo=null;
...
function closeStage(){stage.hidden=true;stageSend.hidden=true;storyVideo=null;stageBar.style.width='0%'}
stageClose.addEventListener('click',()=>{closeStage();audio.pause()});
...
stageSend.addEventListener('click',async()=>{if(!storyVideo)return;try{await navigator.share({files:[storyVideo],title:card.title,text:card.title+' · '+card.url});closeStage()}catch(error){if(!error||error.name!=='AbortError')stageNote.textContent='That could not be shared. Try Save instead.'}});
...
async function movingCard(clip){stage.hidden=false;stageSend.hidden=true;stageNote.textContent='Recording your story…';audio.pause();
const blob=await storyCard.recordStoryVideo({canvas:stageCanvas,card,audio,clipStart:clip.start,seconds:clip.seconds,onProgress:value=>{stageBar.style.width=(value*100).toFixed(1)+'%'}});
storyVideo=new File([blob],card.videoFileName,{type:blob.type||'video/mp4'});
stageNote.textContent='Ready. This opens your share sheet.';stageSend.hidden=false}
story.addEventListener('click',async()=>{if(storyBusy)return;storyBusy=true;clearTimeout(storySettle);
// A recording that fails leaves the stage rather than sharing: the gesture is
// spent by then, so it cannot fall through to the still card's share sheet.
const clip=storyRecordable?storyClipNow():null;
try{if(clip&&clip.seconds>=5){storyLabel.textContent='…';await movingCard(clip);storyLabel.textContent=storyResting}else await stillCard()}
