...
${STORY_CARD_SCRIPT}
let card=song.card;
const story=document.querySelector('#story'),storyLabel=story.querySelector('span'),parts=Array.isArray(song.parts)?song.parts:[];
const stage=document.querySelector('#story-stage'),stageCanvas=document.querySelector('#story-canvas'),stageParts=document.querySelector('#story-parts'),stageBar=document.querySelector('#story-bar'),stageNote=document.querySelector('#story-note'),stageGo=document.querySelector('#story-go'),stageClose=document.querySelector('#story-close');
...
let storyDrawing=null,storyBusy=false,storySettle=0,storyVideo=null,storyChips=[];
// Opens on the part the card already quotes, so the first thing a person sees
// is the card they would have got anyway.
let storyPart=Math.max(0,parts.findIndex(part=>part.sectionIndex===card.sectionIndex));
...
// The clip plays the part the card quotes, measured against the real audio
// rather than the duration the provider claimed.
function storyClipNow(){return storyCard.storyClip(timeline,card.sectionIndex,audio.duration,storyPart,parts.length)}
function storyLength(){const seconds=storyClipNow().seconds;return seconds>=1?'Record '+Math.round(seconds)+'s':'Record'}
async function drawPreview(){const image=await storyCard.loadStoryBackground(card.backgroundUrl);storyCard.drawStoryCard(stageCanvas,card,image)}
// Picking a part moves the words and the audio together, so the card can never
// quote one part of the song while playing another.
function choosePart(index){storyPart=index;const part=parts[index];card={...card,sectionIndex:part.sectionIndex,stanza:part.stanza};storyDrawing=null;storyVideo=null;
storyChips.forEach((chip,at)=>chip.setAttribute('aria-pressed',at===index?'true':'false'));
stageBar.style.width='0%';stageNote.hidden=true;stageGo.hidden=false;stageGo.textContent=storyLength();
drawPreview().catch(()=>{})}
function buildChips(){if(storyChips.length||parts.length<2)return;storyChips=parts.map((part,index)=>{const chip=document.createElement('button');chip.type='button';chip.className='story-part';chip.textContent=part.label;chip.setAttribute('aria-pressed','false');chip.addEventListener('click',()=>{if(!storyBusy)choosePart(index)});stageParts.append(chip);return chip})}
function setChipsEnabled(enabled){storyChips.forEach(chip=>{chip.disabled=!enabled})}
function closeStage(){stage.hidden=true;storyVideo=null;stageNote.hidden=true;stageBar.style.width='0%';audio.pause()}
stageClose.addEventListener('click',closeStage);
async function record(){const clip=storyClipNow();if(!(clip.seconds>=5)){stageNote.hidden=false;stageNote.textContent='This part is too short to record.';return}
stageGo.hidden=true;setChipsEnabled(false);stageNote.hidden=false;stageNote.textContent='Keep this open · '+Math.round(clip.seconds)+'s';
...
catch{stageBar.style.width='0%';stageNote.hidden=false;stageNote.textContent='That recording stopped. Try again.';stageGo.textContent=storyLength();stageGo.hidden=false}
finally{setChipsEnabled(true)}}
