...
function validateTimelineDuration(){const duration=audio.duration,tolerance=Math.max(1,duration*.02);timingValidated=Boolean(timeline&&Number.isFinite(duration)&&duration>0&&Math.abs(song.expectedDurationSeconds-duration)<=tolerance);if(!timingValidated)timeline=null;timingNote.textContent=timingValidated?'Lines follow MiniMax sections · timing is approximate':'Atmospheric reveal · timing is approximate';renderKey=''}
