...
function closeStage(){stage.hidden=true;stageSend.hidden=true;stageNote.hidden=false;storyVideo=null;stageBar.style.width='0%'}
...
stageSend.addEventListener('click',async()=>{if(!storyVideo)return;try{await navigator.share({files:[storyVideo],title:card.title,text:card.title+' · '+card.url});closeStage()}catch(error){if(!error||error.name!=='AbortError'){stageNote.hidden=false;stageNote.textContent='That could not be shared. Close and try Save.'}}});
...
async function movingCard(clip){stage.hidden=false;stageSend.hidden=true;stageNote.hidden=false;stageNote.textContent='Keep this open while it records';audio.pause();
const blob=await storyCard.recordStoryVideo({canvas:stageCanvas,card,audio,clipStart:clip.start,seconds:clip.seconds,onProgress:value=>{stageBar.style.width=(value*100).toFixed(1)+'%';const left=Math.ceil(clip.seconds*(1-value));stageNote.textContent='Keep this open · '+left+'s'}});
storyVideo=new File([blob],card.videoFileName,{type:blob.type||'video/mp4'});
stageNote.hidden=true;stageSend.hidden=false}
