...
const lines=Array.isArray(song.lines)?song.lines:[],sections=Array.isArray(song.sections)?song.sections:[];
...
function makeSection(section){const element=document.createElement('span');element.className='lyric-section lyric-section-current';element.setAttribute('aria-current','true');section.lines.forEach(value=>element.append(makeLine(value)));return element}
...
function syncSections(){const entry=activeEntry(audio.currentTime),nextKey=entry?entry.start+':'+entry.end+':'+entry.label+':'+entry.sectionIndex:'timed-rest';if(renderKey===nextKey)return;renderKey=nextKey;const section=entry&&entry.sectionIndex!==null?sections.find(value=>value.index===entry.sectionIndex):null;if(section)linesRoot.replaceChildren(makeSection(section));else if(entry&&(entry.label==='inst'||entry.label==='silence'))linesRoot.replaceChildren(makeLine({cue:true,primary:entry.label==='inst'?'Instrumental':'Pause',secondary:''}));else linesRoot.replaceChildren()}
function validateTimelineDuration(){const duration=audio.duration,tolerance=Math.max(1,duration*.02);timingValidated=Boolean(timeline&&Number.isFinite(duration)&&duration>0&&Math.abs(song.expectedDurationSeconds-duration)<=tolerance);if(!timingValidated)timeline=null;timingNote.textContent=timingValidated?'Section timing from MiniMax analysis':'Atmospheric reveal · timing is approximate';renderKey=''}
