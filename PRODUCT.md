...
- **Lyric timing is section-level, and says so.** Section timing comes from MiniMax's own analysis of the finished recording and is only used when it maps onto the written sections and matches the audio's length within `max(1 s, 2 %)`. Inside those boundaries, romanized syllable weights pace a current-line emphasis at render time; for inline audio bytes, a best-effort band-limited onset check may hold the first section's sung lines until the likely vocal entry. Both are approximate display heuristics, never line, word, or karaoke timing. Timed mode reads exactly `Lines follow MiniMax sections · timing is approximate`; every other case keeps the approximate reveal and its existing copy. Remote URLs and shared pages skip onset analysis, and playback never waits for it. The provider artifact stays immutable per recording and versioned by `version`/`mode`; only normalized section boundaries are stored — never derived pacing, onset results, provider transcripts, trace ids, feature ids, or signed URLs.
