...
- **Lyric sync is section-level, and says so.** Section timing comes from MiniMax's own analysis of the finished recording and is only used when it maps onto the written sections and matches the audio's length within `max(1 s, 2 %)`. It is not line, word, or karaoke sync, and no surface may claim otherwise: timed mode reads `Section timing from MiniMax analysis`, and every other case keeps the approximate reveal and its existing copy. The artifact is immutable per recording, versioned by `version`/`mode`, and only the normalized boundaries are ever stored — never the provider's transcript, trace id, feature id, or signed URL.
