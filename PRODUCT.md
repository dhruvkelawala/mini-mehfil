...
- **Synchronization is section-level and best-effort:** after an HTTPS Music 3 result, the proxy may use the same key to obtain validated MiniMax section boundaries. Playback follows those boundaries from the media element's current position only when provider and browser durations agree; otherwise it preserves the atmospheric approximate reveal. The product must never describe this as line-, word-, syllable-, or karaoke synchronization.
...
5. Honest timing over fabricated precision — show an original lyric section when MiniMax analysis supports it, clear sung lyrics for instrumental or quiet spans, and fall back rather than interpolate lines.
