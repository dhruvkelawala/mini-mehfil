...
On Vercel, `vercel.cjs` reads `MEHFIL_SHARE_URL` at build time and reverse-proxies `/s/:path*` to that Worker without changing the browser URL. Share responses use Vercel's `VERCEL_PROJECT_PRODUCTION_URL`, so preview and production deployments both copy the canonical production link. Keep automatic Vercel system environment variables enabled. For another host, set `MEHFIL_PUBLIC_URL` to an HTTPS origin explicitly.
